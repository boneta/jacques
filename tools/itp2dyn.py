...
class GMXTopology():
    ...
    def gen_opls(self) -> None:
        ...
        for molname, molec in self.moleculetype.items():
            # atoms in residue
            top['residues'][molname] = DynamoTopology.empty_param('residues')
        ...
        for ptype, missing in self.missing.items():
            for miss in missing:
                if miss in self.opls.top[ptype]:
                    ...
                    self.found.top[ptype][miss] = DynamoTopology.empty_param(ptype, comment=" WARNING: Parameter not found")
